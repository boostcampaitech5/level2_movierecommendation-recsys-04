...
class Trainer:
    def __init__(
        ...
        self.args = args
        self.cuda_condition = (
            torch.cuda.is_available() and not self.args.no_cuda
        )
    ...
    def submission(self, epoch):
        return self.iteration(
            epoch, self.submission_dataloader, mode="submission"
        )
    ...
    def cross_entropy(self, seq_out, pos_ids, neg_ids):
        ...
        seq_emb = seq_out.view(
            -1, self.args.hidden_size
        )  # [batch*seq_len hidden_size]
        ...
        if mode == "train":
            ...
            for i, batch in rec_data_iter:
                ...
                loss = self.cross_entropy(
                    sequence_output, target_pos, target_neg
                )
                self.optim.zero_grad()
                loss.backward()
                self.optim.step()
